...
if choice == "Store Secret":
    ...
    if st.button("Encrypt and Save"):
     ...
     label = st.text_input("Enter Lable:")
     passkey = st.text_input("Enter Paskey" ,type="passkey")

     if st.button("Decrypt"):
          conn = sqlite3.connect("simple_data.db")
          c = conn.cursor()
          c.execute("SELECT encrypted_text, passkey FROM value WHERE label=?", (label,))
          result = c.fetchone
          conn.close()
